# REA-IA-PPGE
📘 Descrição
Este Recurso Educacional Aberto (REA) foi desenvolvido para alunos de licenciatura na disciplina Tecnologias Digitais e Recursos Educacionais, do Programa de Pós-Graduação em Educação (PPGE) da PUCPR.
Apresenta o uso da ferramenta SciSpace e suas aplicações na pesquisa e escrita acadêmica, incentivando a reflexão sobre o uso ético, crítico e pedagógico da inteligência artificial na formação docente.

🎯 Objetivos
- Compreender o funcionamento e as potencialidades do SciSpace;
- Explorar o uso de IA na produção e revisão de textos acadêmicos;
- Desenvolver uma postura crítica diante do uso de tecnologias generativas;
- Promover práticas pedagógicas inovadoras apoiadas em IA.

🧩 Público-alvo
Estudantes de licenciatura, professores em formação e educadores interessados em compreender como a inteligência artificial pode contribuir na pesquisa, escrita e ensino de maneira ética e reflexiva.

💡 Como utilizar
- Acesse os materiais disponíveis neste repositório (guias, exemplos e reflexões).
- Explore a ferramenta SciSpace e realize as atividades propostas.
- Registre suas percepções sobre as potencialidades e os desafios do uso da IA na educação.
- Compartilhe e adapte este material conforme suas necessidades educativas.

🧭 Estrutura do REA
📂 /materiais — textos, tutoriais e exemplos de uso do SciSpace
🧠 /reflexoes — discussões e estudos sobre ética e IA na educação
📝 /atividades — propostas práticas para sala de aula e formação docente

🔍 Licença
Este material é disponibilizado sob a Licença Creative Commons Atribuição 4.0 Internacional (CC BY 4.0).
Você pode copiar, distribuir, remixar e adaptar este conteúdo, desde que cite a autoria e mantenha a mesma licença.

✨ Autoria
Produzido por Guiullianna Drapcynski Bot
Disciplina: Tecnologias Digitais e Recursos Educacionais
Programa de Pós-Graduação em Educação (PPGE) — PUCPR
